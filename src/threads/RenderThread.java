...
public class RenderThread extends ThreadPerso{
	...
	public RenderThread(int period, GamePanel gamePanel, StatusBarPanel statusBarPanel, HightScoresPanel hightScoresPanel,MusicThread musicTh, SoundThread soundTh) {
		...
		physicsTh = new PhysicsThread(maze.getMazeValues(), gamePanel, pacMan, blinky, pinky, clyde, inky, setPacDots(), setEnergizers(), musicTh , soundTh);
	...
	private void gameRender()
	{
		if (drawnOnce){
			...
			synchronized (pacDots) {
				if(physicsTh.isLevel()) {
					pacDots=setPacDots();
					physicsTh.setPacDots(setPacDots());
					energizers=setEnergizers();
					physicsTh.setEnergizer(setEnergizers());
					physicsTh.setLevel(false);
				}
				pacDots.draw(dbg); 
			}
			...
			if(physicsTh.isNextLevel()) {
				levelThread = new LevelThread(maze.getTiles(), gamePanel, physicsTh.getScoreLevelI());
