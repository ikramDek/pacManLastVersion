...
public class GhostsExitBoxThread extends TimerThread {
	...
	private boolean manageGhostExit(Ghost ghost) { 
		if(!pacMan.invincible()) {
			...
			if(ghost.getCurrentPosition().getX() > 270  && ghost.getCurrentPosition().getX() < 275) {
			...
			else if(ghost.getCurrentPosition().getX() < 275) {
				ghost.setWantedState(MovingSpriteState.RIGHT);
				ghost.setState(ghost.getWantedState());
			}
			else if(ghost.getCurrentPosition().getX() > 270 && ghost.getCurrentPosition().getX() < 350) {
